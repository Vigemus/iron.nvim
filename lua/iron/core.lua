...
core.send_line = function()
  ...
  local width = vim.fn.strlen(cur_line)
...
core.mark_visual = function()
  ...
  if mode == "\22" then
    ...
    for ix, line in ipairs(lines) do
      ...
      lines[ix] = vim.fn.strpart(line, b_offset , math.min(e_col, vim.fn.strlen(line)))
    end
  elseif mode == "v" then
    local last = #lines
    local line_size = vim.fn.strlen(lines[last])
    ...
    if (max_width < line_size) then
      ...
      lines[last] = vim.fn.strpart(lines[last], 0, max_width)
    end
...
core.mark_motion = function(mtype)
  ...
  if mtype=='line' then
    b_col, e_col = 0, vim.fn.strlen(lines[#lines])
  end
...
core.send_mark = function()
  ...
  if #lines == 1 then
    if pos.from_col >= 1 or pos.to_col < vim.fn.strlen(lines[1]) - 1 then
    ...
    if pos.to_col < vim.fn.strlen(lines[#lines]) - 1 then
